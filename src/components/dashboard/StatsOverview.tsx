...
const StatsOverview = () => {
  ...
  const totalValue = DUMMY_PRODUCTS.reduce((sum, product) => sum + (product.price * product.stock), 0);
  const lowStockCount = DUMMY_PRODUCTS.filter(p => p.status === 'low_stock').length;
  const outOfStockCount = DUMMY_PRODUCTS.filter(p => p.status === 'out_of_stock').length;
  const inStockCount = DUMMY_PRODUCTS.filter(p => p.status === 'in_stock').length;

  // Get top products by value
  const topProducts = DUMMY_PRODUCTS
    .map(p => ({ ...p, totalValue: p.price * p.stock }))
    .sort((a, b) => b.totalValue - a.totalValue)
    .slice(0, 5);

  // Category distribution
  const categoryStats = DUMMY_PRODUCTS.reduce((acc, product) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-foreground mb-2">Statistik Inventory</h2>
        <p className="text-muted-foreground">
          Ringkasan status stok dan performa inventory
        </p>
      </div>
